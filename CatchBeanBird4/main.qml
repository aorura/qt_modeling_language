    ...
    Flipable {
       ...
       front/* fill the blank */: Rectangle {
        ...
        Component.onCompleted/* fill the blank */: {
            ...
            for(i = 0; i < 25; i++) {
                var gridStr = "import QtQuick 2.0; Rectangle { width: rootRect.width/5-4; height: width; border { width: 2; color: \"black\"; } }"
                var dynamicObject1 = Qt.createQmlObject(gridStr, board, 'gridObject'+i);

                var boxStr = "import QtQuick 2.0; Rectangle { \
                                                      anchors { fill: parent; margins: 2 } clip: true; }"
                var dynamicObject2 = Qt.createQmlObject(boxStr, dynamicObject1, 'gridObject'+i);
        ...
        Timer {
            ...
            onTriggered: {
                ...
                for (var i=0; i < 10; i++) {
                    if (score.model.get(i).score <= catchedBird) {
                        score.model.insert(i, {"name": "", "score": catchedBird});
                        break;
                    }
                };
